...
use criterion::{Criterion, BatchSize, Bencher, criterion_group, criterion_main};
...
type Fischlin<C> = FischlinTransform<DualProofOfExponentSigmaProtocol<C, Sha3_256>>;
type Sigma<C> = DualProofOfExponentSigmaProtocol<C, Sha3_256>;

fn bench_prove<C: CurveAffine, N: NIZK<X=CurvePair<C>, W=FieldPair<C::Scalar>>>(b: &mut Bencher) {
    b.iter_batched(
        || {
            ...
            let (a, b) = (C::Scalar::random(&mut rng), C::Scalar::random(&mut rng));
            (CurvePair(C::one().mul(a).into_affine(), C::one().mul(b).into_affine()), FieldPair(a, b), rng)
        },
...
fn bench_verify<C: CurveAffine, N: NIZK<X=CurvePair<C>, W=FieldPair<C::Scalar>>>(b: &mut Bencher) {
    b.iter_batched(
        || {
            let mut rng = Sha3_256::query(&[
                0x2a, 0xb1, 0x74, 0x52, 0x0f, 0x19, 0x34, 0x2a,
            ...
            let (a, b) = (C::Scalar::random(&mut rng), C::Scalar::random(&mut rng));
            let (x, w) = (CurvePair(C::one().mul(a).into_affine(), C::one().mul(b).into_affine()), FieldPair(a, b));
